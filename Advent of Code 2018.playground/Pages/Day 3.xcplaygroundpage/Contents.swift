    ...
    static func withOverlap(grids: [Grid]) -> [Grid] {
        ...
        for (index, grid) in gridsToReturn.enumerated() {
            ...
            while secondIndex < gridsToReturn.count {
                ...
                /*
                let firstXStart = grid.xOffset
                let firstXEnd = grid.xOffset + grid.width
                let firstYStart = grid.yOffset
                let firstYEnd = grid.yOffset + grid.height
                
                let secondXStart = secondGrid.xOffset
                let secondXEnd = secondGrid.xOffset + secondGrid.width
                let secondYStart = secondGrid.yOffset
                let secondYEnd = secondGrid.yOffset + secondGrid.height
                */
...
//                    let arraySet = Array(intersection)
//                    let sortedArray = arraySet.sorted()
//                    print("\(grid) intersects with \(secondGrid) at cells:\n\(sortedArray)")
            ...
            /*
             // Using a filter would actually be less efficient because it would filter every grid every time, which you don't have to do.
            let overlappingGrids = gridsToReturn.enumerated().filter { (offset, secondGrid) -> Bool in
                if false { // TODO: compare grid to secondGrid
                    
                }
                return false // TODO: return correct conditions
            }
            */
        }
...
do {
    /*
    let testGrid = try Grid(instructions: "#123 @ 3,2: 5x4")
    print(testGrid.visualDescription)
    */
    /*
    let grid1 = try Grid(instructions: "#1 @ 1,3: 4x4")
    let grid2 = try Grid(instructions: "#2 @ 3,1: 4x4")
    let grid3 = try Grid(instructions: "#3 @ 5,5: 2x2")
    */
//    print(grid1, grid2, grid3)
//    let overlappingGrids = Grid.overlappingVisual(grids: [grid1, grid2, grid3])
//    let overlappingInches = overlappingGrids.filter { $0 == "X" }
//    print(overlappingGrids)
    /*
    print(overlappingInches.count)
    print(Grid.overlapping(grids: [grid1, grid2, grid3])) // Doesn't fully work at the moment because it doesn't mark the first grid as overlapping
    */
    /*
    print(Grid.withOverlap(grids: [grid1, grid2, grid3])) // Seems to work
    print(grid1.areaSet(masterWidth: 8))
    print(grid2.areaSet(masterWidth: 8))
    print(grid3.areaSet(masterWidth: 8))
    */
//    print(grid1.areaArrays(masterWidth: 8)) // Confirms that grid1 co-ords are correct
    /*
    print(testGrid.areaArrays(masterWidth: 9))
    print(testGrid.areaArray(masterWidth: 9))
    print(testGrid.areaSet(masterWidth: 9))
    */
} catch let error {
    print(error)
}
...
func part1() throws -> Int {
    ...
    let overlappingGrids = Grid.overlappingVisual(grids: inputGrids)
    let overlappingInches = overlappingGrids.filter { $0 == "X" }
    return overlappingInches.count
}
