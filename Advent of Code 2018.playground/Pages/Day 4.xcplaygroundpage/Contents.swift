...
enum Day4Error: Swift.Error {
    case wrongSplitCount(actualCount: Int)
    case wrongSecondSplitCount(actualCount: Int)
    case cannotParseDate(from: String)
    case cannotParseId(from: String)
    case noEventId(in: Event)
    case noCurrentId(in: Event)
    case fallsAsleepIsLastEvent
    case wakeUpIsNotAfterFallsAsleep(actual: Event)
    case guardDoesNotContainMinuteMostAsleep(guard: Guard)
}
...
struct Event {
    ...
    init(from string: String) throws {
        let split = string.split { character -> Bool in
            return character == "[" || character == "]"
        }
        
        guard split.count == 2 else { throw Day4Error.wrongSplitCount(actualCount: split.count) }
        
        let stringDate = String(split[0])
        
        guard let date = Event.dateFormatter.date(from: stringDate) else { throw Day4Error.cannotParseDate(from: stringDate) }
        
        self.date = date
        
        let stringType = String(split[1])
        ...
        if type == .startShift {
            let secondSplit = stringType.split { character -> Bool in
                return character == "#" || character == " "
            }
            
            guard secondSplit.count == 4 else { throw Day4Error.wrongSecondSplitCount(actualCount: secondSplit.count) }
            
            let stringId = String(secondSplit[1])
            
            guard let id = Int(stringId) else { throw Day4Error.cannotParseId(from: stringId) }
    ...
    static func sortedEvents(from: String) throws -> [Event] {
        let lines = from.split(separator: "\n")
        let strings = lines.map { String($0) }
        let events = try strings.map { try Event(from: $0) }
        let sortedEvents = events.sorted { (event1, event2) -> Bool in
            return event1.date.compare(event2.date) == .orderedAscending
        }
        return sortedEvents
    }
    
    static func minutesBetween(event1: Event, event2: Event) -> Int {
        return Int(event2.date.timeIntervalSince(event1.date)) / 60
    }
    
    func minutesSince(event: Event) -> Int {
        return Int(self.date.timeIntervalSince(event.date)) / 60
    }
    
    func minutesUntil(event: Event) -> Int {
        return Int(event.date.timeIntervalSince(self.date)) / 60
    }
    
    func minutesBetween(event: Event) -> Int {
        return abs(Int(self.date.timeIntervalSince(event.date)) / 60)
    }
    
    func arrayOfMinutesSince(event: Event) -> [Int] {
        let previousMinute = event.minuteFromMidnight
        let thisMinute = minuteFromMidnight
    
        if previousMinute < thisMinute {
            return [Int](previousMinute..<thisMinute)
        } else {
            return []
        }
    }
    
    func arrayOfMinutesUntil(event: Event) -> [Int] {
        let thisMinute = minuteFromMidnight
        let nextMinute = event.minuteFromMidnight
        
        if thisMinute < nextMinute {
            return [Int](thisMinute..<nextMinute)
        } else {
            return []
        }
    }
    
    var minuteFromMidnight: Int {
...
extension Event: CustomStringConvertible {
    var description: String {
        return "Event: Date: \(date), type: \(type), ID: \(id)"
    }
}
...
struct Guard {
    ...
    var minutesAsleepCounts: [Int: Int] {
        var dict: [Int: Int] = [:]
        for minute in minutesAsleep {
            var newMinute: Int {
                if let existingCount = dict[minute] {
                    return existingCount + 1
                } else {
                    return 1
                }
            }
            dict[minute] = newMinute
        }
        return dict
    }
    
    var minuteMostAsleep: (minute: Int?, count: Int?) {
        var mostMinute: Int? = nil
        var minuteCount: Int? = nil
        
        for (minute, count) in minutesAsleepCounts {
            if let existingCount = minuteCount {
...
func part1Tests() throws {
    let testEvents = try Event.sortedEvents(from: testInput)
    testEvents.enumerated().forEach { index, event in
        print(event)
        if index < testEvents.count - 1 {
            let nextEvent = testEvents[index + 1]
            if event.type == .fallsAsleep {
                //                print(Event.minutesBetween(event1: event, event2: nextEvent))
                print(event.minutesBetween(event: nextEvent))
                //                print(nextEvent.arrayOfMinutesSince(event: event))
                print(event.arrayOfMinutesUntil(event: nextEvent))
            }
        }
    }
}
...
func parsedGuards(from eventString: String) throws -> [Guard] {
    let events = try Event.sortedEvents(from: eventString)
    ...
    try events.enumerated().forEach { index, event in
        switch event.type {
        case .startShift:
            guard let eventId = event.id else { throw Day4Error.noEventId(in: event) }
            currentId = eventId
        case .fallsAsleep:
            guard let currentId = currentId else { throw Day4Error.noCurrentId(in: event) }
            guard index < eventsCount - 1 else { throw Day4Error.fallsAsleepIsLastEvent }
            
            let nextEvent = events[index + 1]
            
            guard nextEvent.type == .wakesUp else { throw Day4Error.wakeUpIsNotAfterFallsAsleep(actual: nextEvent) }
            
            let minutesAsleep = event.arrayOfMinutesUntil(event: nextEvent)
            
            if let guardIndex = guards.firstIndex(where: { `guard` -> Bool in
                return `guard`.id == currentId
            }) {
                let existingGuard = guards[guardIndex]
                let newGuard = Guard(id: existingGuard.id, minutesAsleep: existingGuard.minutesAsleep + minutesAsleep)
                guards[guardIndex] = newGuard
            } else {
                let newGuard = Guard(id: currentId, minutesAsleep: minutesAsleep)
                guards += [newGuard]
            }
        case .wakesUp:
            break
...
func minuteMostAsleep(from: [Guard]) throws -> Int {
    let mostAsleepGuard = from[0]
    
    guard let minuteMostAsleep = mostAsleepGuard.minuteMostAsleep.minute else { throw Day4Error.guardDoesNotContainMinuteMostAsleep(guard: mostAsleepGuard) }
    let guardId = mostAsleepGuard.id
    
    let answer = guardId * minuteMostAsleep
    
    print("\(guardId) * \(minuteMostAsleep) = \(answer)")
    
    return answer
}

func part1() throws -> Int {
    let guards = try parsedGuards(from: input)
    
    let sortedGuards = guards.sorted { guard1, guard2 -> Bool in
        return guard1.minutesAsleep.count > guard2.minutesAsleep.count
    }
    
    return try minuteMostAsleep(from: sortedGuards) // 35184, correct!
}
...
/*
let date = Event.dateFormatter.date(from: "1518-11-05 00:55")
print(date)
let modernDate = Event.dateFormatter.date(from: "2018-12-23 10:00")
print(modernDate)


let incorrect = Event.dateFormatter.date(from: "1847-11-30 23:59")!
print(incorrect)
let nonExistant = Event.dateFormatter.date(from: "1847-12-01 00:00")
print(nonExistant)
let alsoNonExistant = Event.dateFormatter.date(from: "1847-12-01 00:01")
print(alsoNonExistant)
let correct = Event.dateFormatter.date(from: "1847-12-01 00:02")!
print(correct)
*/
...
func part2() throws -> Int {
    ...
    let sortedGuards = try guards.sorted { guard1, guard2 -> Bool in
        guard let guard1MostAsleepCount = guard1.minuteMostAsleep.count else { throw Day4Error.guardDoesNotContainMinuteMostAsleep(guard: guard1) }
        guard let guard2MostAsleepCount = guard2.minuteMostAsleep.count else { throw Day4Error.guardDoesNotContainMinuteMostAsleep(guard: guard2) }
        
        return guard1MostAsleepCount > guard2MostAsleepCount
    }
    
    return try minuteMostAsleep(from: sortedGuards) // 37886. correct!
}
